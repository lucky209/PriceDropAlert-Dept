...
public class FlipkartHelper {
    ...
    void downloadFlipkartImages(WebDriver browser, int count, String dept) throws Exception {
        ...
        try {
            ...
            this.findImageElementsAndSave(browser,dept,count,0);
        }
        if (waitElement != null) {
            ...
            for (int i = 0; i < liElements.size(); i++) {
                ...
                this.findImageElementsAndSave(browser, dept, count, i);
            }
            actions.moveToElement(liElements.get(0)).build().perform();
        }
    }

    private void findImageElementsAndSave(WebDriver browser, String dept, int count, int productCount) throws Exception {
        ...
        if (!imgElements.isEmpty()) {
            ...
            String pathToSave = folderPath + (count + 1) + "-" + (productCount + 1) + Constant.IMAGE_FORMAT;
